...
const Add = ({ isOpen, setIsOpen, type, setList }: AddProps) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!source || !amount || !date) {
      setErrorMsg("Please fill all fields, including date.");
      return;
    }
    ...
    const normalized = {
      ...
      category:
        type === "income" ? res.data.income_source : res.data.expense_category,
  ...
  return (
    <div
      ...
      className="absolute inset-0 flex justify-center items-center bg-gray-900/20 "
    >
      <div
        onClick={(e) => e.stopPropagation()}
        className="w-[50%] max-w-[500px] bg-white rounded-lg"
      >
        <div className="flex justify-between items-center border-b border-b-gray p-5">
          <h1 className="font-semibold text-xl">Add {type}</h1>
          <X
            onClick={() => setIsOpen(false)}
            className="text-gray-400 stroke-current text-sm cursor-pointer"
          />
        </div>
        <div className="p-5">
          ...
          <form onSubmit={handleSubmit} className="space-y-4 max-w-md">
            ...
            <Button
              type="submit"
              className="text-right bg-[#f56565] rounded-lg"
            >
              Add {type}
            </Button>
          </form>
